...
import { defaults, denied, type Policy } from './policy.ts';

export const GRAPH_VERSION = 'symbols-v2';
...
export function readSafe(root: string, path: string, maxBytes = 1048576): Buffer {
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > maxBytes) throw new Error('Only regular files up to 1 MiB are supported');
    const bytes = readFileSync(fd);
    if (bytes.length > maxBytes || bytes.includes(0)) throw new Error('Binary or oversized file excluded');
    return bytes;
  } finally { closeSync(fd); }
}

export function manifest(root: string, policy: Policy = defaults): { files: FileFact[]; skipped: string[] } {
  const paths = [...new Set(localGit(root, 'files').split('\0').filter(Boolean))].sort();
  if (paths.length > policy.maxFiles) throw new Error('Tracked file count exceeds policy.maxFiles');
  const files: FileFact[] = [];
  const skipped: string[] = [];
  for (const path of paths) {
    if (denied(path, policy) || !policy.extensions.includes(extname(path).toLowerCase()) || !extensions.has(extname(path).toLowerCase()) || path.split('/').some(p => excluded.has(p))) {
      skipped.push(path); continue;
    }
    try {
      const bytes = readSafe(root, path, policy.maxFileBytes);
